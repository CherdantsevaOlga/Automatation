...
def test_create_and_get_employee():
    ...
    db.insert_employee("Kerry", "Mitchell", "+7556564425", new_company_id)
    ...
    assert any(employee["firstName"] == "Kerry" and employee["lastName"] == "Mitchell" for employee in employee_list)
...
def test_update_employee():
    ...
    db.update_employee(employee_id, "Jo", "Pitt", "Middle", "+987654321", "jane.doe@example.com", "http://example.com")

    updated_employee = db.get_employee_by_id(employee_id)  # Получаем обновленного сотрудника напрямую
    assert updated_employee["first_name"] == "Jo"
    assert updated_employee["last_name"] == "Pitt"
